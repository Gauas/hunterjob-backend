...
import (
	...
	httptransport "github.com/hunterjob/hunterjob/api/internal/transport/http"
...
func main() {
	...
	defer cancel()
	s, client, e := store.New(ctx, cfg.MongoURI, cfg.Database)
	if e != nil {
		log.Fatal(e)
	}
	defer client.Disconnect(context.Background())
	if e = s.SeedTechvify(context.Background()); e != nil {
		log.Fatal(e)
	}
	r := gin.New()
	...
	admin := httptransport.NewAdminHandler(appsource.NewService(s, q))
	var aiProvider appsearch.Provider = infraai.Deterministic{}
	if cfg.AIProvider == "openai" {
		aiProvider = infraai.OpenAI{Key: cfg.OpenAIKey, Model: cfg.OpenAIModel}
	}
	aiSearch := httptransport.NewSearchHandler(appsearch.NewService(s, aiProvider))
